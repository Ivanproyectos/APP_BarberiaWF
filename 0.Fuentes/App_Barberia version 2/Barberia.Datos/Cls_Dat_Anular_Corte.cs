...
{
    public class Cls_Dat_Anular_Corte : Repository<T_ANULAR_CORTE>
    {
        public List<T_ANULAR_CORTE> Listar_Anular_Corte(ref Cls_Ent_Auditoria auditoria)
        {
            ...
            try
            {
                ...
                //{
                //    lista = db.T_ANULAR_CORTE.Where(x => x.FLG_ESTADO == "1").OrderByDescending(x => x.ID_CARGO).ToList();
                //}
                lista = GetAll().ToList();
            }
            catch (Exception ex)
        ...
        public List<T_ANULAR_CORTE> BuscarReporte_Anular_Corte(string fechaInicio, string fechaFin, ref Cls_Ent_Auditoria auditoria)
        {
            List<T_ANULAR_CORTE> lista = new List<T_ANULAR_CORTE>();
            auditoria.Limpiar();
            IQueryable<T_ANULAR_CORTE> query = Entities;
            try
            {
        ...
        {
            T_ANULAR_CORTE lista = new T_ANULAR_CORTE();
            auditoria.Limpiar();
            bool exito = true;
            try
            {
